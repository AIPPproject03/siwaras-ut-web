...
async function generatePDF() {
  ...
  try {
    ...
    infoData.forEach((row) => {
      doc.text(row[0], margin, infoY);
      doc.text(row[1], margin + 40, infoY);
      doc.text(row[2], margin + 45, infoY);
      infoY += 6;
    });
    ...
    doc.setFont("helvetica", "normal");
    doc.text(currentTandaTerima.createdBy || "Admin", col1X, yPos);
    doc.text(currentFormData.nama || "-", col2X, yPos);
